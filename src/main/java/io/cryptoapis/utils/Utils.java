...
public class Utils {
    ...
    public static Map.Entry<String, ApiError> setQueryParams(Map<String, String> params) {
        URIBuilder builder = new URIBuilder();
        if (params != null) {
            for (Entry<String, String> param : params.entrySet()) {
                ApiError apiError = Utils.checkQueryParams(param.getKey().toLowerCase());
                if (apiError != null) {
                    return new SimpleEntry(null, apiError);
                }
        ...
        try {
            ...
            return new SimpleEntry(null, Utils.convertToApiError(Utils.setError(e.getMessage(), 1)));
        }

        if (uri == null) {
            return new SimpleEntry<>(null, Utils.convertToApiError(Utils.setError("Try again later", 1)));
        }
        return new SimpleEntry<>(uri.toString(), null);
    }
